...
description: F5 brand font families, type hierarchy, weight usage, leading specs, case conventions, and Starlight theme mapping.
sidebar:
  order: 1
---

F5's type system blends two font families inspired by Swiss design, establishing a clear hierarchy between expressive headlines and functional body copy.

Source: [F5 Brand Center — Fonts and typography](https://brand.f5.com/d/yYzHXTsZ4EGq/elements#/-/fonts-and-typography)

## Design Philosophy

F5 typography uses proportion, rhythm, and tempo — achieved through type style, scale, weight, color, leading, horizontal space, and vertical space — to create clear visual hierarchy.
...
The downloadable font package from the Brand Center (`Neusa-Next-Pro-F5-Inhouse-Type.zip`) also includes **Bold Italic** (700 italic), for a total of six OTF source files. This is an F5 in-house typeface — not available from public font services.
...
- Sign-offs and CTAs use **Neusa Wide Bold** with a right-facing caret (**›**) in F5 Red to indicate action in digital experiences
...
| Pull quote | Neusa Wide | Bold (700) | Sentence | F5 Red color; large red quotation marks with attribution below |
| Sign-off / CTA | Neusa Wide | Bold (700) | Sentence | With right-facing caret (›) in F5 Red |
...
- **`h1`–`h6`** → Font family, weight, and case applied per the F5 type hierarchy
...
| `--sl-line-height-headings` | `1.1` | F5 brand headline leading |
...
### Brand Center Download

The [F5 Brand Center fonts page](https://brand.f5.com/d/yYzHXTsZ4EGq/elements#/-/fonts-and-typography) provides a downloadable package containing `Neusa-Next-Pro-F5-Inhouse-Type.zip` with six OTF source files:

| OTF File | Weight | Style |
|----------|--------|-------|
| `NeusaNextPro-WideLight.otf` | Light (300) | Normal |
| `NeusaNextPro-WideRegular.otf` | Regular (400) | Normal |
| `NeusaNextPro-WideItalic.otf` | Regular (400) | Italic |
| `NeusaNextPro-WideMedium.otf` | Medium (500) | Normal |
| `NeusaNextPro-WideBold.otf` | Bold (700) | Normal |
| `NeusaNextPro-WideBoldItalic.otf` | Bold (700) | Italic |

To convert OTF to woff2, use Python's `fonttools` library:

```python
from fontTools.ttLib import TTFont
font = TTFont("NeusaNextPro-WideItalic.otf")
font.flavor = "woff2"
font.save("neusaNextProWide-400i.woff2")
```

### Proxima Nova

Proxima Nova is a commercially licensed typeface by Mark Simonson Studio. It is **not** included in the Brand Center download and must be obtained through a separate license. The theme ships with normal-style woff2 files for weights 400, 500, 600, and 700.
